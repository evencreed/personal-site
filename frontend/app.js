// Tema + arkaplan çizgiler + mouse izi + iletişim API
...
(function background(){
  ...
  let W=0, H=0;
  let lines = [];
  const trail = []; // mouse iz parçacıkları
  ...
  function step(){
    // silerken hafif iz bırak (motion blur)
...
document.addEventListener('DOMContentLoaded', ()=>{
  ...
  form?.addEventListener('submit', async (e)=>{
    ...
    try{
      if(company) { status.textContent = 'Bot algılandı.'; return; }
      btn && (btn.disabled = true);
      status.textContent = 'Gönderiliyor…';

      await sendMessage({ name, email, message, company });
      status.textContent = 'Mesaj gönderildi, teşekkürler!';
      form.reset();
    }catch(err){
      console.error(err);
      status.textContent = 'Mesaj gönderilemedi: ' + err.message;
    }finally{
